...
namespace edm {
  ...
  public:
    ...
    EventID nextRun(LuminosityBlockNumber_t const& lumi) const { return EventID(run_ + 1, lumi, 0); }
    EventID nextRunFirstEvent(LuminosityBlockNumber_t const& lumi) const { return EventID(run_ + 1, lumi, 1); }
    ...
    bool operator<(EventID const& iRHS) const {
      ...
      if (run_ > iRHS.run_)
        return false;
      if (luminosityBlock_ < iRHS.luminosityBlock_)
        return true;
      if (luminosityBlock_ > iRHS.luminosityBlock_)
        return false;
    ...
    bool operator>=(EventID const& iRHS) const { return !(*this < iRHS); }

    bool operator==(EventID const& iRHS) const { return !(*this < iRHS || iRHS < *this); }

    bool operator!=(EventID const& iRHS) const { return !(*this == iRHS); }

    bool operator<=(EventID const& iRHS) const { return (*this < iRHS || *this == iRHS); }
    ...
    static EventNumber_t maxEventNumber() { return 0xFFFFFFFFFFFFFFFFULL; }

    static EventID firstValidEvent() { return EventID(1, 1, 1); }
    // ---------- member functions ---------------------------

    void setLuminosityBlockNumber(LuminosityBlockNumber_t const& lb) { luminosityBlock_ = lb; }
  ...
  inline EventID const& min(EventID const& lh, EventID const& rh) { return (rh < lh ? rh : lh); }

  inline EventID const& max(EventID const& lh, EventID const& rh) { return (rh < lh ? lh : rh); }
}  // namespace edm
#endif
